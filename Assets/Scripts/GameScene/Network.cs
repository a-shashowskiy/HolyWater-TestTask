...
#pragma warning restore CS4014
            ...
            if (request.result != UnityWebRequest.Result.ConnectionError) { progress?.Invoke(1f); }
            return request;
        }
        ...
        public static async Task<string> GetText(string url)
        {
            ...
            if (uwr != null && uwr.result != UnityWebRequest.Result.ConnectionError  && uwr.result != UnityWebRequest.Result.ProtocolError)
            {
        ...
        public static async Task<byte[]> GetData(string url, CancellationTokenSource cancelationToken, System.Action<float> progress = null)
        {
            ...
            if (uwr != null && uwr.result != UnityWebRequest.Result.ProtocolError && uwr.result != UnityWebRequest.Result.ConnectionError)
            {
        ...
        public static async Task<AssetBundle> GetAssetBundle(string url, string urlManifest, CancellationTokenSource cancelationToken, System.Action<float> progress = null, bool caching = true)
        {
            UnityWebRequest request;
            CachedAssetBundle assetBundleVersion = await GetAssetBundleVersion(urlManifest);
